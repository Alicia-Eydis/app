...
def main():
    ...
    if uploaded_file is not None:
        ...
        if classNo == 0:
            st.subheader(f"これは{pred0}％の確率で「一花」です！")
        elif classNo == 1:
            st.subheader(f"これは{pred1}％の確率で「二乃」です！")
        elif classNo == 2:
            st.subheader(f"これは{pred2}％の確率で「三玖」です！")
        elif classNo == 3:
            st.subheader(f"これは{pred3}％の確率で「四葉」です！")
        else:
            st.subheader(f"これは{pred4}％の確率で「五月」です！")
...
# 学科：AIシステム
# 学年：2年
# コメント：五等分の花嫁のヒロイン達の判別をおこなったが、
# 　　　　　特定の画像を認識させるとちゃんと判別できなかった。
# （課題に取り組んだ感想や不明点、工夫した点など記入してください）
#
#=============================================================================#
